...
船代智能问答助手核心智能体。采用三阶段循环架构：
  Plan（规划解答路径）→ Execute（依次执行工具）→ Reflect（自我反思）
  
特性：
  - 7 个专业化工具（实体提取、规章检索、追问用户、术语查询、SQL查询、合规校验、答案生成）
  - 结构化认知状态（实体注册表、规章匹配、置信度、缺失约束追踪）
  - 自反思 / 质量批评循环
  - 置信度驱动的自动路由
  - 完整 SSE 事件兼容
...
from .db_settings import ELASTICSEARCH_URI, ELASTICSEARCH_INDEX
...
class AgentState(TypedDict):
    ...
    books: list                     # 用户选择的知识库来源

    # ── 业务认知 ──
    triage_result: dict             # 意图分类结果 {"intent","urgency","department","brief_summary"}

    symptoms: list                  # 结构化实体表（字段名保留 symptoms 以兼容 Prompt 模板变量）
                                    # [{"name","type","value","confidence","related_context"}]

    tcm_patterns: list              # 规章匹配结果（字段名保留 tcm_patterns 以兼容 Prompt 模板变量）
                                    # [{"regulation_name","confidence","supporting_evidence","contradicting_evidence","applicable_scope","key_requirements"}]

    diagnosis_hypotheses: list      # 多源证据鉴别排名
                                    # [{"diagnosis","probability","key_supporting","key_against","distinguishing_points"}]

    information_gaps: list          # 缺失约束 ["目的港","货物类型",...]

    safety_flags: list              # 合规标记
                                    # [{"level","description","suggestion"}]

    primary_pattern: str            # 主要适用规章/结论
    ba_gang: dict                   # 查询分类 {"query_type","involves_codes","involves_calculation"}

    # ── 循环控制 ──
    diagnostic_plan: list           # 当前解答计划 ["extract_entities","search_regulations",...]
    ...
    action: str
    action_input: dict
    confidence_score: float         # 整体答案置信度 (0~1)
...
class ReActAgent:
    """高级 Plan-Execute-Reflect 船代智能问答智能体"""

    # 合法的 action 名称
    VALID_ACTIONS = {
        "extract_entities", "search_regulations", "ask_user",
        "query_terminology", "execute_sql", "check_compliance", "generate_answer"
    }

    def __init__(self, redis_url: str, session_id: str, elasticsearch_client=None, terminology_client=None):
        ...
        if self.terminology_client:
            print("✅ [ReActAgent] 术语库客户端已注入")
        else:
            print("⚠️ [ReActAgent] 术语库客户端未注入，query_terminology 工具将降级")
    ...
    def _tool_triage(self, state: AgentState) -> dict:
        ...
        try:
            ...
            if result:
                print(f"  ✓ 意图: {result.get('intent')} | 紧急度: {result.get('urgency')} | 方向: {result.get('department')}")
                return {"triage_result": result}
        except Exception as e:
            print(f"  ✗ Triage 失败: {e}")

        return {"triage_result": {"intent": "规章检索", "urgency": "medium", "department": "集装箱运输", "brief_summary": state["query"][:50]}}

    # ══════════════════════════════════════════════════════════════
    #  工具 2: Extract Entities — 结构化实体提取
    # ══════════════════════════════════════════════════════════════

    def _tool_extract_entities(self, state: AgentState) -> dict:
        """从自然语言中提取结构化业务实体"""
        print("\n🔬 [Extract Entities] 正在提取结构化实体...")
        try:
            ...
            if result:
                ...
                gaps = result.get("missing_constraints", [])
                ...
                print(f"  ✓ 提取到 {len(entities)} 个新实体 (总计 {len(merged)})")
                print(f"  ✓ 缺失约束: {', '.join(merged_gaps[:5])}")
                print(f"  ✓ 初步判断: {impression}")
    ...
    # ══════════════════════════════════════════════════════════════
    #  工具 3: Search Regulations — 混合检索
    # ══════════════════════════════════════════════════════════════

    def _tool_search_regulations(self, state: AgentState) -> dict:
        """混合检索（ES BM25 + Dense bge-m3）+ LLM Reranking"""
        action_input = state.get("action_input", {})
        search_query = action_input.get("query", state["query"])
        source_filter = action_input.get("sources", state.get("books", []))
        top_k = action_input.get("top_k", 10)
        top_n = min(top_k, 5)

        print(f"\n📚 [Search Regulations] 检索: \"{search_query}\"")
        all_new_docs = []

        # Elasticsearch 混合检索（BM25 + Dense）
        if self.elasticsearch_client:
            try:
                sparse_results = self.elasticsearch_client.search_sparse(search_query, top_k=top_k, book_filter=source_filter)
                sparse_docs = self._process_sparse_results(sparse_results)
                all_new_docs.extend(sparse_docs)
                print(f"  ✓ ES 混合检索: {len(sparse_docs)} 条")
            except Exception as e:
                print(f"  ✗ ES 检索失败: {e}")
    ...
    def _tool_ask_user(self, state: AgentState) -> dict:
        """基于缺失约束生成专业追问"""
        action_input = state.get("action_input", {})
        ...
        if not question:
            ...
            entities = state.get("symptoms", [])

            if gaps:
                gap_text = "、".join(gaps[:4])
                question = f"为了更准确地为您解答，我还需要了解以下信息：{gap_text}。请您补充说明。"
            elif len(entities) < 2:
                question = "请您提供更多细节——包括具体的船名/航次、起运港/目的港、货物类型、涉及的业务环节等，以便我给出更精准的答复。"
            else:
                question = "请问您这个问题涉及的具体操作时间节点是什么？是否有需要遵守的特殊港口要求或客户合同约束？"

        reason = action_input.get("reason", "信息不足")

        print(f"\n❓ [Ask User] 追问: {question}")
        print(f"   原因: {reason}")
    ...
    # ══════════════════════════════════════════════════════════════
    #  工具 5: Query Terminology — 术语库查询
    # ══════════════════════════════════════════════════════════════

    def _tool_query_terminology(self, state: AgentState) -> dict:
        """通过术语库进行同义归一、编码映射和规则校验"""
        print("\n📖 [Query Terminology] 正在查询术语库...")
        try:
            entities_detail = json.dumps(state.get("symptoms", []), ensure_ascii=False, indent=2)
            retrieved_context = self._format_context(state.get("retrieved_docs", []))

            prompt = tcm_pattern_prompt_template.format(
                symptoms_detail=entities_detail,
                retrieved_context=retrieved_context
            )
            result = self._parse_json_response(self._call_llm(prompt))
            if result:
                regulations = result.get("regulation_matches", [])
                primary = result.get("primary_regulation", "")
                classification = result.get("query_classification", {})
                summary = result.get("analysis_summary", "")

                print(f"  ✓ 匹配到 {len(regulations)} 条规章")
                print(f"  ✓ 主要规章: {primary}")
                print(f"  ✓ 查询分类: {classification}")
                print(f"  ✓ 分析: {summary[:100]}...")

                # 计算置信度
                max_conf = max([p.get("confidence", 0) for p in regulations]) if regulations else 0

                return {
                    "tcm_patterns": regulations,
                    "primary_pattern": primary,
                    "ba_gang": classification,
                    "confidence_score": max_conf,
                }
        except Exception as e:
            print(f"  ✗ 术语查询/规章分析失败: {e}")

        return {}

    # ══════════════════════════════════════════════════════════════
    #  工具 6: Execute SQL — 结构化数据查询
    # ══════════════════════════════════════════════════════════════

    def _tool_execute_sql(self, state: AgentState) -> dict:
        """通过 LLM 生成 SQL 查询结构化数据（费率表、船期表等）"""
        print("\n🔍 [Execute SQL] 正在查询结构化数据...")
        try:
    ...
    # ══════════════════════════════════════════════════════════════
    #  工具 7: Check Compliance — 合规性校验
    # ══════════════════════════════════════════════════════════════

    def _tool_check_compliance(self, state: AgentState) -> dict:
        """业务方案合规性检查"""
        print("\n🛡️ [Check Compliance] 正在进行合规性校验...")
        try:
            conclusion = state.get("primary_pattern", "未确定")
            entities_detail = json.dumps(state.get("symptoms", []), ensure_ascii=False)

            prompt = safety_check_prompt_template.format(
                diagnosis=conclusion,
                treatment_principle=f"针对{conclusion}的标准操作流程",
                symptoms_detail=entities_detail,
                patient_info="从对话历史中推断的用户业务背景"
            )
            result = self._parse_json_response(self._call_llm(prompt))
            if result:
                flags = result.get("compliance_flags", result.get("safety_flags", []))
                warnings = result.get("user_warnings", [])
                risk = result.get("overall_risk", "low")
                is_safe = result.get("is_safe", True)

                print(f"  ✓ 合规: {'是' if is_safe else '否'} | 风险等级: {risk}")
                for flag in flags:
    ...
    def _reflect(self, state: AgentState) -> dict:
        ...
        try:
            ...
            if result:
                ...
                return {
                    "action": next_action if next_action in self.VALID_ACTIONS else "ask_user",
                    "thought": critique,
                }
        except Exception as e:
            print(f"  ✗ 反思失败: {e}")
    ...
    def _generate_plan(self, state: AgentState) -> list:
        ...
        try:
            ...
            prompt = plan_prompt_template.format(
                ...
                department=triage.get("department", "集装箱运输"),
                query=state["query"],
                symptoms_summary=self._summarize_entities(state.get("symptoms", [])),
                patterns_summary=self._summarize_regulations(state.get("tcm_patterns", [])),
        ...
        entities = state.get("symptoms", [])
        if not entities:
            return ["extract_entities", "ask_user"]
        return ["search_regulations", "query_terminology", "generate_answer"]
    ...
    def _reason_node(self, state: AgentState) -> dict:
        ...
        try:
            ...
            human = react_human_template.format(
                query=state["query"],
                books=", ".join(state.get("books", [])) or "全部知识库",
                history=self._format_chat_history(state.get("chat_history", [])),
            ...
            print(f"  ✗ 推理失败: {e}")
            return {"thought": str(e), "action": "generate_answer", "action_input": {}}
    ...
    async def react_stream(self, info: dict) -> AsyncIterator[dict]:
        ...
        try:
            ...
            # Phase 0: Triage — 意图路由
            # ════════════════════════════════════════
            step_triage = {"icon": "🔖", "title": "意图路由", "details": "正在分析您的问题类型和紧急程度..."}
            ...
            step_triage["details"] = f"意图: {triage.get('intent', '规章检索')} | 方向: {triage.get('department', '集装箱运输')} | 紧急: {triage.get('urgency', 'medium')}"
            ...
            while iteration < max_iterations:
                ...
                # ──── Phase 1: Plan ────
                step_plan = {"icon": "📋", "title": f"制定解答计划（第{iteration}轮）", "details": "正在规划下一步解答路径..."}
                ...
                for step_idx, tool_name in enumerate(plan):
                    ...
                    if tool_name == "ask_user":
                        step_ask = {"icon": "❓", "title": "追问用户", "details": "正在基于缺失信息生成专业追问..."}
                        ...
                        ask_result = await asyncio.get_event_loop().run_in_executor(None, self._tool_ask_user, state)
                        state.update(ask_result)
                        state["executed_tools"].append("ask_user")
                    ...
                    # 特殊处理: generate_answer 结束循环
                    if tool_name == "generate_answer":
                        # 先执行合规校验（如果还没做过）
                        if "check_compliance" not in state["executed_tools"]:
                            step_compliance = {"icon": "🛡️", "title": "合规性校验", "details": "正在检查方案的合规性..."}
                            thinking_steps.append(step_compliance)
                            yield {"thinking_step": step_compliance}

                            compliance_result = await asyncio.get_event_loop().run_in_executor(None, self._tool_check_compliance, state)
                            state.update(compliance_result)
                            state["executed_tools"].append("check_compliance")

                            flags = state.get("safety_flags", [])
                            step_compliance["details"] = f"发现 {len(flags)} 个合规提醒" if flags else "合规校验通过 ✓"
                            yield {"thinking_update": step_compliance}

                        # 质量审核
                        step_critique = {"icon": "🧐", "title": "质量审核", "details": "正在进行答案质量审核..."}
                    ...
                    # ── 普通工具执行 ──
                    tool_configs = {
                        "extract_entities":   ("🔬", "实体结构化提取", self._tool_extract_entities),
                        "search_regulations": ("📚", "检索知识库",     self._tool_search_regulations),
                        "query_terminology":  ("📖", "术语库查询",     self._tool_query_terminology),
                        "execute_sql":        ("🔍", "结构化数据查询", self._tool_execute_sql),
                        "check_compliance":   ("🛡️", "合规性校验",     self._tool_check_compliance),
                    }
                    ...
                    if tool_name in tool_configs:
                        ...
                        # 如果是 search_regulations，先用 ReAct 推理确定检索关键词
                    ...
                    # ──── Phase 3: Reflect ────
                    step_reflect = {"icon": "🪞", "title": "自我反思", "details": "正在审视解答过程的完备性..."}
                    ...
                    next_action = reflect_result.get("action", "generate_answer")
                    ...
                    # 根据反思结果决定：继续循环 or 结束
                    if next_action == "generate_answer" and state.get("confidence_score", 0) >= 0.6:
                        break
                    if next_action == "ask_user":
                        state["diagnostic_plan"] = ["ask_user"]
                        continue  # 回到 Plan，但 plan 里只有 ask_user

                    # 其他情况继续循环到下一轮 Plan
                    continue

                # 如果从 generate_answer 的 break 出来，也退出主循环
                break

            # ════════════════════════════════════════
            # Phase Final: 流式生成最终答案
            # ════════════════════════════════════════

            step_final = {
                "icon": "✅",
                "title": "生成专业解答",
                "details": f"基于 {len(state.get('symptoms', []))} 个实体 · {len(state.get('retrieved_docs', []))} 条资料 · 结论: {state.get('primary_pattern', '综合分析')}"
            }
            ...
            if not context or context == "暂无检索资料。":
                context = "暂无检索资料，请基于您的行业经验回答。"

            # 增强上下文：附加规章分析和合规信息
            enhanced_context = context
            if state.get("primary_pattern"):
                enhanced_context += f"\n\n【结论】主要适用规章/结论: {state['primary_pattern']}"
            if state.get("ba_gang"):
                bg = state["ba_gang"]
                enhanced_context += f"\n查询分类: 类型={bg.get('query_type','?')} 涉及编码={bg.get('involves_codes','?')} 涉及计算={bg.get('involves_calculation','?')}"
            if state.get("safety_flags"):
                sf = state["safety_flags"]
                enhanced_context += f"\n\n【合规提醒】共 {len(sf)} 条合规提醒：" + "; ".join([f.get("description", "") for f in sf])
    ...
    def _parse_react_output(self, text: str, default_sources: list) -> tuple:
        """解析 ReAct 格式输出 → (thought, action, action_input)"""
        thought, action, action_input = "", "generate_answer", {}
        try:
            ...
            if action == "search_regulations":
                action_input.setdefault("sources", default_sources)
                action_input.setdefault("query", thought[:50] if thought else "")
            if action == "ask_user" and "question" not in action_input:
                action_input["question"] = thought or "请提供更多业务细节"
        except Exception as e:
            print(f"  ⚠️ 解析失败: {e}")
            thought = text[:200]
    ...
    def _rerank_documents(self, query: str, docs: List[Dict], top_n: int = 5) -> List[Dict]:
        ...
        try:
            if len(docs) <= top_n:
                return docs
            from core.templates.prompt_registry import doc_processor_prompt
            doc_texts = []
            for i, doc in enumerate(docs):
                t = f"文档[{i+1}]: "
                if doc.get("is_image"): t += "[图表] "
                t += doc["text"][:200]
    ...
    def _process_sparse_results(results: list) -> List[Dict]:
        return [{"text": h.get("text", ""), "page_number": h.get("page_number", h.get("section_path", 0)),
                 "is_image": h.get("is_image_description", False),
                 "score": h.get("score", 0), "source": "elasticsearch"} for h in results]
    ...
    def _format_context(docs: List[Dict]) -> str:
        ...
        for i, d in enumerate(docs):
            t = f"[文档{i+1}] "
            if d.get("is_image"): t += "[图表描述] "
            t += d["text"]
            if d.get("page_number"): t += f" (位置: {d['page_number']})"
            t += f" [来源: {d.get('source', '?')}]"
            parts.append(t)
        return "\n\n".join(parts)
    ...
    def _summarize_entities(entities: list) -> str:
        if not entities: return "暂无"
        return ", ".join([f"{s.get('name', '?')}({s.get('confidence', 0):.0%})" for s in entities[:8]])

    @staticmethod
    def _summarize_regulations(regulations: list) -> str:
        if not regulations: return "暂无"
        return ", ".join([f"{p.get('regulation_name', p.get('pattern_name', '?'))}({p.get('confidence', 0):.0%})" for p in regulations[:5]])
    ...
    def _get_tool_detail(tool_name: str, state: AgentState) -> str:
        ...
        if tool_name == "extract_entities":
            entities = state.get("symptoms", [])
            gaps = state.get("information_gaps", [])
            return f"提取到 {len(entities)} 个实体 | 缺失约束: {', '.join(gaps[:4]) or '无'}"
        elif tool_name == "search_regulations":
            return f"检索到 {len(state.get('retrieved_docs', []))} 条相关资料"
        elif tool_name == "query_terminology":
            p = state.get("primary_pattern", "未确定")
            c = state.get("confidence_score", 0)
            return f"主要规章: {p} (置信度: {c:.0%})"
            ...
            flags = state.get("safety_flags", [])
            return f"{len(flags)} 个合规提醒" if flags else "合规校验通过 ✓"
        return "完成"
